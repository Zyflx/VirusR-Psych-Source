...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite();
		bg.frames = Paths.getSparrowAtlas('Background');
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				...
				gfDance.frames = Paths.getSparrowAtlas('R_title2');
				gfDance.animation.addByPrefix('idle', 'Rbiaoti', 32, true);
		...
		dtflogo = new FlxSprite(0, FlxG.height * 0.52 + 10).loadGraphic(Paths.image('DaytimefishStudio_logo'));
		add(dtflogo);
		dtflogo.visible = false;
		dtflogo.setGraphicSize(Std.int(dtflogo.width * 0.8));
		dtflogo.updateHitbox();
		dtflogo.screenCenter(X);
		dtflogo.antialiasing = ClientPrefs.globalAntialiasing;
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					...
					#end*/
					createCoolText(['Virus R Mod', 'By']);
				case 8:
					// addMoreText('newgrounds', -40);
					addMoreText('DayTimefish Studio');
					dtflogo.visible = true;
					// ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					deleteCoolText();
